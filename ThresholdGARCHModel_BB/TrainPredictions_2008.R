...
legend("center",c("Observed-SWHft","Predicted-SWHft","NA-Indicator"),col=c("black","indianred3","yellow2"),pch=19,cex=1.5)
i=1
plot(Data$TimeStamp_1[which(Data$MM==MonthWindyNum[i])][-(1:5)],Data$SWHft[which(Data$MM==MonthWindyNum[i])][-(1:5)],type="l",
     ylim=c(0,10),main =paste(MonthWindy[i],(j-1),sep="-"))
xlab("Time")
...
{
  plot(Data$TimeStamp_1[which(Data$MM==MonthWindyNum[i])],Data$SWHft[which(Data$MM==MonthWindyNum[i])],type="l",
       ylim=c(0,10),main =paste(MonthWindy[i],j,sep="-"))
  lines(Data$TimeStamp_1[which(Data$MM==MonthWindyNum[i])],Data$SWHft[which(Data$MM==MonthWindyNum[i])],
        pch=19,cex=0.2,type="p",col=Data$colSWHft[which(Data$MM==MonthWindyNum[i])])
  lines(Data$TimeStamp_1[which(Data$MM==MonthWindyNum[i])],PredInt[which(Data$MM==MonthWindyNum[i])],
        col="indianred3")
...
legend("center",c("Observed-SWHft","Predicted-SWHft","NA-Indicator"),col=c("black","indianred3","yellow2"),pch=19,cex=1.5)
i=1
plot(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])][-(1:4)],Data$SWHft[which(Data$MM==MonthCalmNum[i])][-(1:4)],
     type="l",ylim=c(0,10),main =paste(MonthCalm[i],j,sep="-"))
lines(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])][-(1:4)],Data$SWHft[which(Data$MM==MonthCalmNum[i])][-(1:4)],col=Data$colSWHft[which(Data$MM==MonthCalmNum[i])][-(1:5)],pch=19,cex=0.2,type="p")
lines(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])][-(1:4)],PredInt[which(Data$MM==MonthCalmNum[i])][-(1:4)],col="indianred3")
lines(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])][-(1:4)],PredInt[which(Data$MM==MonthCalmNum[i])][-(1:4)],col="indianred3",pch=20,cex=0.2)
for(i in 2:length(MonthCalm))
{
  plot(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])],Data$SWHft[which(Data$MM==MonthCalmNum[i])],type="l",ylim=c(0,10),
       main =paste(MonthCalm[i],j,sep="-"))
  lines(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])],Data$SWHft[which(Data$MM==MonthCalmNum[i])],col=Data$colSWHft[which(Data$MM==MonthCalmNum[i])],pch=19,cex=0.2,type="p")
  lines(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])],PredInt[which(Data$MM==MonthCalmNum[i])] ,col="indianred3")
  lines(Data$TimeStamp_1[which(Data$MM==MonthCalmNum[i])],PredInt[which(Data$MM==MonthCalmNum[i])],col="indianred3",pch=20,cex=0.2)
}
