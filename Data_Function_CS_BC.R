...
#Wind Data from Coastal Station
load("Sikorsky-Bridgeport_merged1.Rda")
...
data.in_CS_bc<-function(whichYear,indSeason)
{
  library(lubridate)
  library(dplyr)
  #For Wind Data
  Data1<-data.frame(seq(ymd_hms("1973-1-1 00:00:00"),ymd_hms(" 2004-12-31 23:00:00"),by="hours"))
  ...
  EventData <-read.csv("~/Dropbox/Namitha Pais/Marine Sciences/Storm_Data_Filtered_1973_2004.csv")
  ...
  #Adjusting for bad values of windspeed
  #th<-7
  #rWSPD_1=data.in$rWSPD
  #for(i in 3:length(data.in$rWSPD))
  #{
    #if(abs(data.in$rWSPD[i]-data.in$rWSPD[i-1])>=th)
    #{
      #data.in$rWSPD[i]<- mean(c(data.in$rWSPD[i-1],data.in$rWSPD[i+1]))
    #} else{
      #data.in$rWSPD[i]<-data.in$rWSPD[i]
    #}
  #}
...
save("data.in_CS_bc", file="data_fn_CS_backcast.Rdata")
