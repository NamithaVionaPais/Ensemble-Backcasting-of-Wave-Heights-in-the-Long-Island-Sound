...
#whichBuoy:central or western
#whichYear:2004-2013
#indSeason:Annual-Annual Data,Windy-Windy Season Data,Calm-Calm Season Data
data.in<-function(whichBuoy,whichYear,indSeason)
{
  set.seed(88699)
  
  ###########################################################################
  ####     This part assumes data has been merged and regularized        ####
  ###########################################################################
  
  
  filled = merged #data frame
  firstBuoyObs = min(which(!is.na(filled$WVHT)))
  maxWaterGap = 24*7 #slow moving water temp trend
  maxAirGap = 24*1 #faster moving air temp trend
  maxWaveGap = NA #tbd
  maxWindGap = NA #tbd
  
  #remove extreme noise - WTMP
  lookBack = 7*24
  for (i in (lookBack + 1):dim(filled)[1]) {
    check = filled$WTMP[(i-lookBack):(i-1)]
    meanCheck = mean(check, na.rm = TRUE)
    sdCheck = sd(check, na.rm = TRUE)
    # check for nonsense
    if ( ( !is.na(filled$WTMP[i]) & 
           !is.na(meanCheck)  &
           !is.na(sdCheck) & 
           (sdCheck > 0) & 
           ( (abs(filled$WTMP[i] - meanCheck)/sdCheck) > 4.5)
    ) ) {
      filled$WTMP[i] = NA # drop as an error
    }
  }
  
  emptyDat = cbind(seq(1:length(filled$WTMP)),filled$WTMP)
  emptyDat = emptyDat[!is.na(emptyDat[,2]),]
  sss = smooth.spline(emptyDat[,1],emptyDat[,2])
  emptyResponse = filled$WTMP
  emptyResponse[is.na(emptyResponse)] = predict(sss,(1:length(filled$WTMP))[is.na(filled$WTMP)])$y
  filled$WTMP = emptyResponse
  
  
  #~~~~ATMP
  
  #remove extreme noise - ATMP
  lookBack = 1*24
  for (i in (lookBack + 1):dim(filled)[1]) {
    check = filled$ATMP[(i-lookBack):(i-1)]
    meanCheck = mean(check, na.rm = TRUE)
    sdCheck = sd(check, na.rm = TRUE)
    # check for nonsense
    if ( ( !is.na(filled$ATMP[i]) & 
           !is.na(meanCheck)  &
           !is.na(sdCheck) & 
           (sdCheck > 0) & 
           ( (abs(filled$ATMP[i] - meanCheck)/sdCheck) > 4.5)
    ) ) {
      filled$ATMP[i] = NA # drop as an error
    }
  }
  ...
  #~~~~Wind
  lookBack = 6 #hr
  for (i in (lookBack + firstBuoyObs):dim(filled)[1]) {
    bufferWSPD = filled$WSPD[(i-lookBack):(i-1)]
    bufferWDIR = filled$WDIR[(i-lookBack):(i-1)]
    #missing any portion of wind
    if ( is.na(filled$WSPD[i]) | is.na(filled$WDIR[i]) ) {
      lastGood = max(which((is.na(bufferWSPD)*is.na(bufferWDIR)) == 0))
      filled$WSPD[i] = bufferWSPD[lastGood]
      filled$WDIR[i] = bufferWDIR[lastGood]
    }
  }
  ...
  #CHECK!!!
  #lookBack = 6 #hr
  #for (i in (firstBuoyObs + 1):dim(filled)[1]) {
  #bufferWVHT = filled$WVHT[(i-lookBack):(i-1)]
  #missing any portion of wind
  #if ( is.na(filled$WVHT[i]) ) {
  #lastGood = max(which(!is.na(bufferWVHT)))
  #filled$WVHT[i] = bufferWVHT[lastGood]
  #}
  #}
  
  ###########################################################################
  ####    Pick a year, decompose wind, #set threshold# and run INLA        ####
  ###########################################################################
  
  #Obtaining hourly time stamps
  library(lubridate)
  library(dplyr)
  ...
  filled<-Data_New_1
  filled$YYYY<-year(filled$TimeStamp_1)
  filled$MM<-month(filled$TimeStamp_1)
  filled$DD<-day(filled$TimeStamp_1)
  filled$hh<-hour(filled$TimeStamp_1)
  filled$mm<-minute(filled$TimeStamp_1)
  
  
  #to be used on post-interpolation files
  #take 1 year
  library(imputeTS)
  data.in_CY = filled[filled$YYYY==whichYear,]
  ...
  data.in_PY=filled[filled$YYYY==(whichYear-1),]
  ...
  #drop observations with missing points
  #goodObs = !(is.na(data.in$WSPD)) & !(is.na(data.in$WDIR)) & !(is.na(data.in$ATMP)) & !(is.na(data.in$Flag)) & !(is.na(data.in$WTMP))
  
  #data.in = data.in[goodObs,]
  
  #temperature attributes
  data.in$CenterTemp = data.in$ATMP - mean(data.in$ATMP)
  data.in$TempDiff = data.in$ATMP - data.in$WTMP
  ...
  DataWindy<-data.in[WindyInd,]
  DataCalm<-data.in[CalmInd,]
  data.in$rWSPDind=as.factor(ifelse(data.in$rWSPD<=0.25,1,0))
...
save("data.in", file="data_fn.Rdata")
